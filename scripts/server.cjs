...
const upload = multer({
  ...
  fileFilter: (req, file, callback) => {
    ...
    if (!allowedTypes.includes(file.mimetype)) {
      console.log(`[server] Archivo rechazado: ${file.mimetype} - ${file.originalname}`);
      callback(new Error(`Tipo de archivo no permitido: ${file.mimetype}`));
      return;
    }
    
    console.log(`[server] Archivo aceptado: ${file.mimetype} - ${file.originalname}`);
    callback(null, true);
  }
});
...
server.post("/api/uploads", (req, res) => {
  upload.single("image")(req, res, (err) => {
    if (err) {
      console.error(`[server] Error al subir imagen:`, err);
    ...
    console.log(`[server] Imagen guardada: ${req.file.filename} (${req.file.size} bytes)`);
...
server.listen(port, "0.0.0.0", () => {
  ...
  console.log(`Uploads ready at http://0.0.0.0:${port}/uploads`);
});
